ls# USAGE: copy.number.distributions.R to:
...
## Define paths
path.main <- "/scratch/mfernandez/wildCatCNV/CNV/"
...
for (i in 1:length(samples)) {
	print(samples[i])
	## Import	
	path.import1 <- paste(path.main,"/mrCNVR/", samples[i],"/calls/", samples[i], ".calls.cw_norm.bed", sep = "")
	path.import2 <- paste(path.main,"/mrCNVR/", samples[i],"/calls/", samples[i], ".calls.copynumber.bed", sep = "")
